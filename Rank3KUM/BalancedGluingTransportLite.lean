...
def gluingContractGroundEquiv (M : Matroid α) (X : Set α) :
    (Matroid.contract M X).E ≃ (M.E \ X : Set α) :=
  Equiv.setCongr (by simp)

/-- The restriction ground is the restricted set. -/
def gluingRestrictGroundEquiv (M : Matroid α) (X : Set α) :
    (Matroid.restrict M X).E ≃ X :=
  Equiv.setCongr (by simp)

/--
Second balanced-gluing ablation.  This is the same semantic route as
`exists_cyclicBasisOrder3_of_nonempty_proper_tight_gluing`, but it avoids the
hand-built restriction/contraction ground equivalences and uses Mathlib's
ground equalities through `Equiv.setCongr` instead.
-/
theorem exists_cyclicBasisOrder3_of_nonempty_proper_tight_gluing_transportLite
    (M : Matroid α) (k : ℕ)
    (hk : 0 < k)
    (hE : M.E.Finite)
    (hRank : M.eRank = 3)
  ...
  · -- rank-one tight set: points are `X`, pairs come from `M / X`.
    ...
    let : Fintype (Matroid.contract M X).E :=
      Fintype.ofEquiv (M.E \ X : Set α)
        (gluingContractGroundEquiv M X).symm
    ...
    have hContractCard :
      ...
      calc
        Fintype.card (Matroid.contract M X).E =
          ...
          Fintype.card_congr (gluingContractGroundEquiv M X)
    ...
    let pairs : Fin k × Bool ≃ (M.E \ X : Set α) :=
      rankTwoOrder.trans (gluingContractGroundEquiv M X)
  ...
  · -- rank-two tight flat: pairs are `X`, points are its complement.
    ...
    let : Fintype (M.restrict X).E :=
      Fintype.ofEquiv X (gluingRestrictGroundEquiv M X).symm
    ...
    have hRestrictCard : Fintype.card (M.restrict X).E = 2 * k := by
      calc
        Fintype.card (M.restrict X).E = Fintype.card X :=
          Fintype.card_congr (gluingRestrictGroundEquiv M X)
    ...
    let pairs : Fin k × Bool ≃ X :=
      rankTwoOrder.trans (gluingRestrictGroundEquiv M X)
    have hpairs_coe (p : Fin k × Bool) :
...
#print axioms Rank3KUM.exists_cyclicBasisOrder3_of_nonempty_proper_tight_gluing_transportLite
